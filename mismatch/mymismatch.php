<?php
  ...
  function draw_bar_graph($width, $height, $data, $max_value, $filename) {
    ...
    $img = imagecreatetruecolor($width, $height);    // Set a white background with black text and gray graphics    
    ...
    imagepng($img, $filename, 5);    imagedestroy($img);
...
*/
  ...
  $query = "SELECT * FROM logins WHERE user_id = '" . $_SESSION['user_id'] . "'";
  $data = mysqli_query($dbc, $query);
  // only possible to find a match for a user who has responded to the questionaire.
  if (mysqli_num_rows($data) != 0) { 
    // this query uses join to select all of the responses for the user.
    ...
    // goes through each row of the query results. builes an array of the users response
    while ($row = mysqli_fetch_array($data)) {
      // when the loop is done, $user_responses array will hold all fo the responses
      array_push($user_responses, $row);
    }

    // Initialize the mismatch search results. tis variable holds the score between two users
    $mismatch_score = 0;
    // user is of the of the person being checked as potential mismatch. will hold id of the best match
    $mismatch_user_id = -1;
    // holds the topics that are mismatched between two users
    ...
    while ($row = mysqli_fetch_array($data)) {
      ...
      $query2 = "SELECT response_id, topic_id, response FROM logins WHERE user_id = '" . $row['user_id'] . "'";
      $data2 = mysqli_query($dbc, $query2);
      $mismatch_responses = array();
      while ($row2 = mysqli_fetch_array($data2)) {
        array_push($mismatch_responses, $row2);
      } // end of iner while loop

      // Compare each response and calculate a mismatch total
      $score = 0;
      $topics = array();
      $categories = array();
      // $i = 0 : starts the loop counter to a value before the looping begins/ $i < ... is the test condition (if $i is < the number of user responses). $i++ : updates the loop counter by 1
      for ($i = 0; $i < count($user_responses); $i++) {    // count returns a count of the number of elements in an array. user response is same number as number of total topics.
        // loop counter is used to step through each user respinse. 
        if ($user_responses[$i]['response'] + $mismatch_responses[$i]['response'] == 3) {
          // score is incremented for each mismatched response 
          $score += 1;
          // each mismatched topic is added to an array so it can be displayed to the user 
